...
\begin{theorem}
\rm \redt{(切比雪夫不等式)} 设随机变量$X$具有数学期望$E(X)=\mu$，方差为$D(X) = \sigma$，则有
$$
P\{|X-\mu| \geq \varepsilon \} \geq \frac{\sigma^2}{\varepsilon}. 
$$
\end{theorem}
...
\begin{definition}
\rm 称$E\{[X-E(X)][Y-E(Y)]\}$为随机变量$X,Y$的\redt{协方差}，记为$\text{Cov}(X,Y)$.
\end{definition}
