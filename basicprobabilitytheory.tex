...
$$
E(\overline{X}) = E(X), D(\overline{X})=\frac{D(X)}{n}.z
$$
\end{proposition}
...
\begin{proposition}
\rm \redt{$\chi^2$的数学期望和方差} 设$\chi_1^2 \sim \chi^2(n_1)$，则
$$
...
\rm 设$X_1,X_2,\cdots,X_n$相互独立，且都服从$N(\mu,\sigma^2)$，则有
$$
\frac{\overline{X} - E(X_i)}{\sqrt{\frac{D(X)}{n}}} \sim N(0,1).
$$
\end{proposition}
...
\begin{proposition}
\rm 设$X_1,X_2,\cdots,X_{n_1}$与$Y_1,Y_2,\cdots,Y_{n_2}$分别来自正态总体分布$N(\mu_1,\sigma_1^2)$及$N(\mu_2,\sigma_2^2)$，且这两个样本相互独立，且有$\sigma_1^2 = \sigma_2^2$，则有
$$
...
\begin{annotation}
\rm \redt{设总体$X \sim N(\mu,\sigma^2)$，检验$H_0: \mu \leq \mu_0$}. 此时对已知$\sigma^2$和未知$\sigma^2$两个情况下选择统计量与检验假设$H_0: \mu = \mu_0$相同. 此时最主要弄清拒绝域的形式，直觉上样本均值$\overline{X}$越小越与原假设相符合，即统计量$U$或$T$越小越符合原假设，因此取$\lambda$使
$$
P\{U \geq \lambda_1\} = \alpha ~\text{或}~P\{T \geq \lambda_2\} =\alpha.
$$
当计算出来的$U$或者$T$分别大于$\lambda_1$或者$\lambda_2$时，则拒绝$H_0$，否则接受$H_0$.
