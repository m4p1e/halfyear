...
\begin{annotation}
\rm {\color{red} 零概率事件$P(A) = 0$与不可能事件$A=\emptyset$不是一个概念}.
\end{annotation}
...
P(AB) = P(A)P(B). 
$$
你可能总想深究其更本质的含义，可能你会联想到当事件$A$和$B$独立时，那么$A$和$B$之间有什么关系呢？ 实际上并{\color{blue}不能确定$A$和$B$有什么特别地确切的关系}，所以你把上述等价理解为在描述一个代数结构就好了，从这个代数结构可以引发一些有趣的性质i.e. 例如化简某些运算{\color{blue}条件概率}，所以就特别地把这个代数结构提出来了. 但是当$P(A)>0, P(B) > 0$时，有这样一种关系: {\color{red} 互斥不独立，独立不互斥}，证明也是很显然的.
\end{annotation}
