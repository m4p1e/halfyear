...
\begin{lemma}
\rm 设real-valued函数$f(x)$在点$x_0$处有有限导数$f'(x_0)$. 若$f'(x) > 0$，则当$x$取右方充分接近$x_0$的数时就有$f(x) > f(x_0)$，而当$x$取左方充分接近于$x_0$的数值时就有$f(x) < f(x_0)$.
\end{lemma}
...
dy = y_x' \cdot \Delta x.
$$
若按照微分的定义，来讨论自变量$x$本身，称它的微分就是增量$\Delta$，即约定
$$
dx = \Delta x.
$$
于是前式可以写作
