...
$$
我们主要关注$U(x)$来说明一定可以取到常数$A$，相当于$U(x)$可以是$u(x)$除$(x-a)$的余数，所以$U(x)$是一定可以取到某个常数$A$.自然地，这里考虑可以取$x = a$消去含$P_1(x)$的项，于是得到$A=\frac{P(a)}{Q_1(a)}$，即$A$是一个常量，然后再确定$Q_1(a)$. 记录一个我自己的疑问{\color{red}为什么给定真分式不能分解为}
$$
...
P(x) = AQ_1(x) + B(x-a)^k, 
$$
按照前面的思路这里$U(x) = u(x) - (x-a)^kt(x)$，所以只能保证$U(x)$取到某个$ax+b$，那么这里$A$可能就不一定是一个常数.

{\color{blue}(2)} 若所给真分式为
$$
\frac{P(x)}{Q(x)} = \frac{P(x)}{(x+px+q)^mQ_1(x)}.
$$
