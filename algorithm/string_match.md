...
$$
其中$\pi^{(i)}[q]=\pi[\pi^{(i-1)}[q]]$，$\pi^{(t)}[q] = 0$.  那么$\pi^*[q] = \{k : P_k \sqsubset P_q\}$.

*Proof.* 两边夹来证$\pi^*[q] \subseteq \{k : P_k \sqsubset P_q\}$是显然.  另一方向，假设$\{k : P_k \sqsubset P_q\} - \pi^*[q] \neq \emptyset$， 取$j$是$\{k : P_k \sqsubset P_q\} - \pi^*[q]$中最大的那个值，可以肯定是必有$j < \pi[q]$，那么我们就可以取$j'$是$\pi^*[q]$中比$j$大的最小值（前面的条件保证了$j'$的存在性），显然$\pi[j'] = j$，因为$j,j' \in \{k : P_k \sqsubset P_q\}$，这和假设是矛盾的.  
