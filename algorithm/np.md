...
对于规模为$n$的输入，在最坏情况下的运行时间是$O(n^k)$，其中$k$为某一确定常数. 
...
**Definition**  给一个判定问题A，若存在一个不同的判定问题B（多项式时间的归约是否需要保证这第二个问题是多项式时间可以解决的），并且存在下面这样的一个过程
...
**Lemma** “多项式时间内能被接受的语言类”是“多项式时间内能被判定的语言类”的一个子集
...
$$
这个$y$是个什么东西?  $y$被称为一个certificate(证书). 例如两个结点的简单路径长度的判定问题，给定一个实例$(G,u,v,k)$，同时给定从一条从$u \to v$的路径$p$，我们可以很容易的检查$p$的长度是否小于等于$k$，如果是，那么就可以把$p$看做一个certificate. 
...
**Definition** 若存在一个多项式时间可计算的函数$f$，满足$x \in L_1$当且仅当$f(x) \in L_2$，则称$f$==归约函数==，计算$f$的多项式时间算法$F$称为==归约算法==.
...
**Definition** 布尔组合电路的规模是指其中布尔组合元素(and,or,not)的数目加上电路中接线的数目. 
...
- 复杂性分类针对某个具体问题所有instances，而不是针对极个别instances. 

- 典型NPC问题
  - 确定$G$上两点之间是否存在一条长度至少为$k$的简单路径（最长路径的decison problem版本）.
  - 电路可满足性
  - 布尔公式可满足性
  - 3-CNF
  - 团问题（完全子图）: 确定$G$是否包含规模为$k$的团.  
  - 顶点覆盖（一组顶点的集合，使得图的每个边至少与集合中的一个顶点相连接）: 确定$G$是否有一个规模为$k$的顶点覆盖
  - 哈密顿回路: 确定$G$是否存在一条哈密顿回路(通过所有顶点的简单回路). 
  - 3-color
  - 旅行商问题:  确定$G$上是否存在一个最大花费为$k$的旅行回路. 
  - 子集和问题: 确定$S$是否存在一个子集其和为$t$. 
  - 子图同构
  - 0-1整数规划问题
  - 集合划分
  - 独立集 （一个顶点子集，$E$中的每条边至多和该子集中一个顶点关联）
- 典型NPH问题
  - 最长路径
  - 最大团
  - 最小顶点覆盖
