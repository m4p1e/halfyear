...
$$
这个$y$是个什么东西?  $y$被称为一个certificate(证书). 例如两个结点的简单路径长度的判定问题，给定一个实例$(G,u,v,k)$，同时给定从一条从$u \to v$的路径$p$，我们可以很容易的检查p的长度是否小于等于$k$，如果是，那么就可以把$p$看做一个certificate. 所以certificate就是一个可以协助我们验证这个实例的辅助信息. 
