...
**Remark** 即使语言$L$被$A$所接受，那么$A$也不会一定就拒绝一个输入$x \in L$.  因为有可能$A$在输入$x$下陷入死循环. 



**Definition** 如果$L$中每个二进制串只是被算法$A$接受或者拒绝，则称语言$L$由算法$A$==判定==. 
...
**Definition** 复杂类NP是能被一个多项式时间算法验证的语言类. 更确切地说，一个语言$L$属于$NP$，当且仅当存在一个多项式验证算法$A$和常数$c$，满足
$$
...
则称$L$是NP完全的.  若$L$仅满足性质1，则称$L$是NP-hard的. 
