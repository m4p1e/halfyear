...
原本的`Adj[u]`表示结点$u$的邻接结点构成的链表.  其所用空间复杂度为$O(n)$，其中$n$表示$u$的邻接结点个数，查找一个邻接结点所需时间为$O(n).$  下面是尝试改进`Adj[u]`的基本数据结构.

1. 如果将`Adj[u]`变为一个固定长度为$|V|$的数组，可以理解为$G$的邻接矩阵表示的第$u$行.  这样查找时间只需要$O(1)$.
2. 如果将`Adj[u]`变为一个哈希表，其所用空间长度相当于用链表表示的$O(n)$，期望查找时间为$O(1)$.  但是最坏的情况是$O(n)$.
3. 如果将`Adj[u]`变为一个二叉搜索树，其空间长度相当于用链表表示的$O(n)$，查找时间为$O(\lg n)$.  维护有成本. 
...
**Definition** 给定一个带权有向无环图$G$

- $G$上的每个结点$v$表示一个事件$v$的发生
- $G$上的每条边$(u,v)$表示事件$u$的发生到事件$v$发生之间的一个活动，权值$w(u,v)$表示活动进行的时间. 
- $G$上有一个入度为$0$的结点$s$，称为源点.
- $G$上有一个出度为$0$的结点$d$，称为汇点.
- 只有在结点$v$所代表的事件发生后，从该结点出发的有向边代表的活动才能开始.  
- 只有在进入结点$v$的所有有向边代表的活动都已经结束之后，才表示结点$v$代表的事件发生. 
- 可以开始的活动之间没有约束，也就是说可以同时执行多个活动. 

这样$G$称为 Activity On Edge Net(AOE网). 



**Definition** AOE网上从源点$s$到汇点$d$的一条简单路径被称为==关键路径==$P$，需要满足

- $P$上任意一条所代表的活动时间增加，则会导致汇点处的事件发生时间增加. 

$P$上的边所代表的活动被称为==关键活动==. 



**Lemma** AOE网上从源点$s$到汇点$d$的最长的简单路径是关键路径$P$. 

*proof.* 这是很trivial的性质，汇点$s$的事件发生，意味着所有从$s$到$d$的简单路径上的活动都必须要执行，最长的路径就是它的一个上界.  



实际上求有向无环图的最长路径的方法有很多，这里更多地是在解决一个实际问题，还是要给出一些和它相关的定义，按照它的实际思路来解决.



下面我们用结点代表事件，边代表活动. 

**Definition** 递归定义结点$v$的最早发生时间
$$
\text{early\_time}(v) = \left\{ \begin{aligned} 
&0  ~& ~\text{$v$是源点}\\
&\max_{(u,v) \in E}\{\text{early\_time}(u) +  w(u,v) \}~ & ~\text{其他}
\end{aligned} \right.
$$
其中$\max$表示结点$v$的发生必须要等到其所有入边完成之后才能发生. 



**Definition** 递归定义结点$v$最晚发生时间
$$
\text{last\_time}(v) = \left\{ \begin{aligned} 
&\text{early\_time}(v)  ~& ~\text{$v$是汇点}\\
&\min_{(v,u) \in E}\{\text{last\_time}(u) -  w(u,v) \}~ & ~\text{其他}
\end{aligned} \right.
$$
注意这里最晚发生时间是保证在==不推迟==整个最短工期的时间下的每个结点的最晚时间.  其中使用$\min$表示结点$v$的发生必须要在它的所有后继结点发生之前.



**Definition** 边$(u,v)$最早开始时间为$\text{early\_time}((u,v)) = \text{early\_time}(u)$.

**Definition** 边$(u,v)$最晚开始时间为$\text{last\_time}((u,v)) =  \text{last\_time}(v)-w(u,v)$.



**Lemma** 关键活动$(u,v)$当且仅当$\text{early\_time}((u,v)) = \text{last\_time}((u,v))$.

*proof.* 上述条件表示活动$(u,v)$必须在某个时间点完成，如果不能完成将影响整体时间. 
...
- 求$\text{early\_time}(v)$
  - 改良版DAG算法，把relax操作换成前面的定义. 
- 求$\text{last\_time}(v)$
  - 类似$\text{early\_time}(v)$，但是这里需要逆的拓扑序. 
- 求$\text{early\_time}((u,v)),\text{last\_time}((u,v))$和关键活动
  - 遍历$V$即可.
...
1. 在对$G^T$进行深搜的时候，可以给每个结点设置一个属性$v.SCC$用于标记它属于两个连通分量. 
...
要生成分量图，当我们做完识别强连通分量之后，我们对$G$再做一次DFS，第一次碰到连接两个不同强连通分量的边我们就可以给它标记一下. 
...
这个推论和前面的lemma是等价，它可以保证我们在$G^T$上深搜的时候，我们对完成时间最大的连通分量进行搜索是safe的，主要确保不会跑到其他的连通分量上去，因为造成这样的边在$G^T$不存在，因为$G^T$上只存在从完成时间较小的连通分量到完成时间较大的边。
...
**Problem** 有向图$G$结点$u$即使有入边和出边，在对$G$进行DFS的时候仍有可能$u$成为一个结点的深搜树.
...
即$(a,b),(b,c),(d,c)$. 先对$d$进行DFS，再对$b$进行DFS，显然$b$是一个结点的深搜树. 

------

**Problem** 广度优先也可以将分为树边，后向边，前向边和横向边. 那么

- 在对无向图进行的广度优先搜索中，下面性质成立
  - 不存在后向边，前向边
  - 对于每条树边$(u,v)$，有$v.d = u.d+1$
  - 对于每条横向边$(u,v)$，有$v.d = u.d$或$v.d = u.d + 1$. 
- 在对有向图进行的广搜中，下面性质成立
  - 不存在前向边
  - 对于每条树边$(u,v)$，有$v.d = u.d+1$
  - 对于每条横向边$(u,v)$，有$v.d \leq v.d + 1$
  - 对于每条后向边$(u,v)$，有$v.d < u.v$. 

------

**Problem** 给定一个有向无环图$G=(V,E)$以及两个结点$s,t$，设计算法给出$s$到$t$不同简单路径的数量.  

对我来说是很有趣的一个问题，最朴素的算法是

```python
simple_paths(u, v)
	if u == v 
		return 1
	else if u.path != NIL #dp思想
		return u.path
	else
    	for each w in Adj[u]
    		u.path = u.paths + simple_paths(w, v)
   		return u.path    
```

这个朴素算法非常简单，但是有可能会遍历很多冗余的边，可以想象所有从$s$可达的结点都会被遍历到，其中有一些结点我们是不需要遍历.  因此我们的改进想法是

- 先从$s$开始做一次dfs. 
- 如果$t$不在深搜树里面，那么就没有$s$到$t$的简单路径.
- 如果$t$在深搜树里面，我们将深搜树上的结点按照$v.f$的时间从小到大排序，取结点$t$和$s$直接的中间结点构成一个集合$S$. $t$到$s$的所有简单路径上的中间结点都会在集合$S$中.
- 在遍历某个结点的之前，我们会先判断该结点是否在$S$中，如果没有那么我们不会去遍历它. 

应该严格证明它. 

------

**Problem** 若有向图$G$上任意两个结点$u,v$之间至多存在一条从$u$到$v$的简单路径，则称$G$是单连通图(singly connected). 设计一个算法判断$G$是否是单连通图. 

- 对某个结点$s$都做一次dfs
- 在dfs的过程中如果发现前向边或者横向边（同一个深度优先树中），则$G$不是一个单连通图.

其时间复杂度为$O(|E|(|V|+|E|))$. 

------

**Problem** 给定一个无向图$G$，设计一个算法在$O(V)$时间上判断$G$是否包含一个环路. 

这里可以使用改良版的DFS，当在DFS的过程中遍历下一个结点的时候新增一个条件判断该结点是否被访问过，如果被访问过那么我们就找到了一个环. 如何确定该算法的运行时间是$O(V)$呢? 上述条件使得我们确保遍历的边数不会超过总的结点数.  

------

**Problem** 拓扑排序的另外一种实现方式: 重复寻找入度为0的结点，输出该结点，同时将改节点和从其出发的边从$G$上删除. 如何在$O(V+E)$时间内实现这种思想. 

- 首先遍历所有结点找到所有入度为0的结点集合$S$，这个过程需要$O(V+E)$时间
- 取$s \in S$，删除$s$和其出边，同时将入度变为0的结点加入$S$. 
- 重复第二个步骤，直到输出所有结点.

如果$G$出现环路，可能会出现$S$为空但是所有结点还有完成输出. 

------

**Problem** 如果在图$G$中加入一条新的边，$G$中的强连通分量会如何变化?

假设添加一条边$(u,v)$

- 如果$u,v$在原$G$的同一个强连通分量中，那么添加$(u,v)$并没有影响
- 如果$u,v$在原$G$的不同的强连通分量中，原本所有强连通分量构成是一个无环图$G'$，此时添加$(u,v)$相对于给两个强连通子图上给这两个强连通分量加了一条边
  - 如果$G'$依然无环，那么强连通分量依然不变
  - 如果$G'$有环形成了，那么意味着可能会将某些原本的强连通分量合并成一个强连通分量，这样强连通分量就减少了
